...
import { Home, Map, NotebookPen, User, Search } from "lucide-react"
...
export function BottomNav() {
  ...
  return (
    <nav aria-label="Päänavigaatio" className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-md">
      <div className="relative border-t border-border bg-card/95 backdrop-blur-md pb-[env(safe-area-inset-bottom)]">
        <div className="flex items-stretch justify-between px-2">
          {left.map((it) => <NavButton key={it.href} {...it} active={isActive(pathname, it.href, it.exact)} />)}
          <div className="w-16 shrink-0" aria-hidden="true" />
          {right.map((it) => <NavButton key={it.href} {...it} active={isActive(pathname, it.href)} />)}
        </div>
        ...
        >
          <Search className="h-9 w-9 text-brass" aria-hidden />
        </Link>
      </div>
    </nav>
  )
...
function NavButton({ href, label, icon: Icon, active }: { href: string; label: string; icon: typeof Home; active: boolean }) {
  return (
    <Link
      href={href}
      className={`flex min-h-14 flex-1 flex-col items-center justify-center gap-1 py-2 text-[11px] font-medium transition-colors ${active ? "text-forest" : "text-muted-foreground"}`}
